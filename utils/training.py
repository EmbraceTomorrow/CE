...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    if "cifar" in args.dataset:
        print(summary(model.net, (3, 32, 32)))
    else:
        print(summary(model.net, (3, 256, 256)))
    ...
    for t in range(0 if args.start_from is None else args.start_from,
        ...
        if args.scheduler is not None:
            if args.scheduler == 'simple':
                ...
                step = None
                if args.dataset == 'seq-cifar100' or args.dataset == 'seq-cifar10':
                    step = [35, 45]
        ...
        if 'ce' in args.model or 'lwfm' in args.model:
            model.add_buffer(train_loader)

        if hasattr(model, 'end_task'):
            model.end_task(dataset)

        if 'ce' in args.model and args.lambda_ftc > 0 and t > 0:
            model.finetune_classifier()

        accs = evaluate(model, dataset, verbose=not model.args.non_verbose)
        print(accs)
        ...
        # save checkpoint
        if 'ce' == args.model:
            path_template = './weights/{}_{}_{}_{}alpha_{}beta_{}lambda_finetune_cls_{}task{}.pth'
            torch.save(model.net.state_dict(), path_template.format(
                model.net.device, dataset.NAME, args.buffer_size, args.alpha, args.beta, args.lambda_ftc,
                t + 1, datetime.datetime.now().strftime(f'%Y-%m-%d')))
